...
　　	pwin.document.write("body {	overflow:hidden;overflow-y:auto;border:0;margin:0;	background-color:white }")
　　	pwin.document.write("body,div {scrollbar-face-color:GREEN;scrollbar-arrow-color:white;scrollbar-highlight-color:white;scrollbar-3dlight-color:green;	scrollbar-shadow-color:white;		scrollbar-darkshadow-color:darkgreen;		scrollbar-track-color:#eef;	}")
　　	pwin.document.write("table {font-size:14px;width:100%;height:1;border-collapse:collapse}")
　　	pwin.document.write("td {border:2px solid black} fieldset td {text-align:center}.label {font-family:黑体}.thinborder td {border:1px solid black}.noborder td {border:0}")
　　	pwin.document.write("#divContent {		width:100%;height:100%;	}	.labelT {font-family:楷体_gb2312;font-weight:bold}	#tdReleaseInfo td {text-align:center}")	
　　	
　　	
　　	pwin.document.write("</style>")
...
<%
	...
	var sButtinId="排放单号"	
	var sOutput="<table id='lb_main'><caption style='font:17px 黑体'>"+cStation+sType+"取样分析排放单</caption>"
...
%>
	<tr><td rowspan=<%=iConfirmFlag%> class=labelT><%=station=="D"?"运行处":"核安全处"%></td>
	...
	<tr><td class=label><%=station=="D"?"值长":"STA"%>：<td><%=Rs("confirm_id")%><td class=label>审批时间：
...
%>
		<br>
			...
			<%
				...
				//if(station=="L"&&sGroup=="LIQ") sKrt=sKrt.replace("904,802")  修改  胡方能
				if(station=="L"&&sGroup=="LIQ") sKrt=sKrt.replace("904","902")
			...
			<td  class=label>实际排放流速(m<sup>3</sup>/h)：<%=GetCol("Release_Speed")%></tr>
			</tr>
			<tr><td class=label>结束排放时间</td><%=GetCol("end_time")%>
			<td  class=label>结束液位(m)：
...
<%if(sStatus=="END") {%>	
	<tr><td style='border:0;text-align:right' COLSPAN=3 >
		<br>
		<%=station=="D"?"副值长签字":"值长签字"%>：__________________
	</td></tr>
<%}%>	
<%	
	CheckExits()
