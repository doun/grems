...
var strIframe = "<html><head><meta http-equiv='Content-Type' content='text/html; charset=gb2312'><style>"+
    ...
    "    <tr><td style='width:1;border-left:1px solid white'><nobr><input name=prevYear title='向前翻 1 年&#13;快捷键：↑' onclick='parent.prevY()' type=button value='|&lt;'"+
    ...
    "    onfocus='this.blur()' name=prevMonth title='向前翻 1 月&#13;快捷键：←' onclick='parent.prevM()' style='border-right:1px solid white' type=button value='&lt;&nbsp;'>"+
    "    </nobr></td><td align=center><input name=today type=button value='今日' onfocus='this.blur()' style='width: 100%;border-right:1px solid white' title='当前日期&#13;快捷键：T'"+
    "    onclick=\"parent.returnDate(new Date().getDate() +'/'+ (new Date().getMonth() +1) +'/'+ new Date().getFullYear())\">"+
    "    </td><td style='width:1;border-right:1px solid white'><nobr><input title='向后翻 1 月&#13;快捷键：→' name=nextMonth onclick='parent.nextM()' style='border-right:1px solid white' type=button value='&nbsp;&gt;'"+
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.iframe     = window.oDHTMLCelendar.document.parentWindow; //日历的 iframe 载体
    ...
    this.darkColor  = "#5b5";    //控件的暗色
    this.lightColor = "#FFFFFF";    //控件的亮色
    this.btnBgColor = "#5b5";    //控件的按钮背景色
    this.wordColor  = "green";    //控件的文字颜色
    this.wordDark   = "orange";    //控件的暗文字颜色
    this.dayBgColor = "#F5F5FA";    //日期数字背景色
    this.todayColor = "#5b5";    //今天在日历上的标示背景色
...
function funMonthSelect() //月份的下拉框
{
    ...
    for (var i=1; i<13; i++) {
		var o2=new WebCalendar.iframe.Option(i +"月",""+ i)
		e.options.add(o2);
	}	
...
function funYearSelect() //年份的下拉框
{
    ...
    for (var i=min; i<=max; i++) e.options.add(new WebCalendar.iframe.Option(i +"年", i));
...
oDateTime.prototype.calendar=function() //主调函数
{
...
oDateTime.prototype.CheckDate=function(e)
{
	...
	//如果日期框为空，返回null；如果日期格式不合法，返回false
	//如果日期格式正确，则返回日期的字符串格式
	if(oInputs.length==3)
	{
