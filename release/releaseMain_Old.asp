...
<script>
	...
	{
		iRsp=parent.tblAnalyze.document.all['release_speed'].innerText
...
<select  name="a" style='width:130'>
<option value="1">CRF泵为1：<%=flow_speed(1)%></option>
<option value="2">CRF泵为2：<%=flow_speed(2)%></option>
<option value="3">CRF泵为3：<%=flow_speed(3)%></option>
<option value="4">CRF泵为4：<%=flow_speed(4)%></option>
</select>
</td></tr>
</table>
</td>
...
<td><input type=text name1=排放流速 id='排放流速' name=Release_Speed  alert='t' datatype='f' <%=oMin()%> max=9e15></td>
<td>液位(m)：</td>
