...
function schange(){
	...
	//var mmonth=document.sform.mm_month.value;
	//var tt=confirm("是否要查看 "+myear+"年"+mmonth+"月 的排放结果");
	//if(tt==true){
		document.sform.action="StatisticMonth.asp";
...
sub showMain()
	...
	LinkStr=LinkStr&"&content="&mdate&"&sdate=M&Stimes=Y"
	
	
	LinkStrV="TER_V1="&TER_V1&"&TER_V2="&TER_V2&"&TER_V3="&TER_V3&""
	LinkStrV=LinkStrV&"&SEL_V1="&SEL_V1&"&SEL_V2="&SEL_V2&"&SEL_V3="&SEL_V3&""
	LinkStrV=LinkStrV&"&content="&mdate&"&sdate=M&V=Y"
	
	p "<table border=0 height=100% width=100% >"
	p "<tr bgcolor=#339933>"
	p "<td height=30>&nbsp;<font color=yellow>矩形图表</font>&nbsp;<a href='StatisticShape.asp?"&LinkStr&"'><font color=yellow>次数</font></a>&nbsp;<a href='StatisticShape.asp?"&LinkStrV&"'><font color=yellow>体积</font></a>&nbsp;&nbsp;<a href='StatisticListeMonth.asp?content="&mdate&"' onclick='return openWin(this.href)'><font color=yellow>输出Excel</font></a>&nbsp;&nbsp;<a href='#'><font color=yellow></font></a>&nbsp;&nbsp;<a href='StatisticListpMonth.asp?content="&mdate&"' onclick='return openWin(this.href)'><font color=yellow>我要打印</font></a></td>"
	...
	for ii=2003 to year(date()) 
		if ii=year(mdate) then
			p "<option value="&ii&" selected>"&ii&"</option>"
		else
			p "<option value="&ii&">"&ii&"</option>"
		end if	
	next
	p "</select>年"
	p "<select name=mm_month onchange='return schange()'>"
	for i=1 to 12 
		counts=right("0"&i,2)
		if i=month(mdate) then
			p "<option value="&counts&" selected>"&counts&"</option>"
		else
			p "<option value="&counts&">"&counts&"</option>"
		end if	
	next
	p "</select>月"
	p "&nbsp;&nbsp;&nbsp;广东核电放射性流出物排放结果&nbsp;&nbsp;&nbsp;&nbsp;"
