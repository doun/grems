...
<%
	...
	{		
		...
		iReturn=sReturn==""?1:0
		sReturn="如果有排放冲突则强制排放"
		/*("
			+"<font style='color:"+(sReturn==""?"red":"")+"'>目前"+sReturn+"有排放冲突</font>)"
			*/
	}	
	else
	...
	function oAdd(oFormInst,dom,data)
	{
		...
		{			
			...
			{	
				...
				{
					if(!bunt("如果有排放冲突，那么将允许排放")) return false						
					iSpecial+=2
				}	
...
<input type=checkbox id=AgreeConflict onclick=checkSpecial(this)>
<label for=AgreeConflict><%=sReturn%></label>　　
</span>
